$('.bt1').on('click', function() {
    ...
    // 시간을 주면 주어진 시간동안 width, height, opacity가 줄어들면서
    //width, height, opacity가 0이되면 사라짐 (display:none)
    $('.box').hide(1000); 
});
...
$('.header-wrapper .navi').on('mouseenter', function () {
    //$(this).childern('.sub-wp').stop().slideDown(1000);   // 자식 선택자
    $(this).find('.sub-wp').stop().slideDown(200); // 자손 선택자
});

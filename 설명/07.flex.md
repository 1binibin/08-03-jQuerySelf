...
justify-content: space-around;       /*자식의 주변으로 균등분할*/
justify-content: space-evenly;       /* 균등분할*/

(세로정렬)
align-items: stretch;                /* [기본값] 자식중 높이가 가장 큰 자식에 모든 자식을 맞춘다*/
align-items: center;                 /* 가운데 맞춘다 (한줄에 쓸때!)*/
align-items: flex-start;             /* 위에 맞춘다*/
align-items: flex-endr;              /* 아래에 맞춘다*/

(줄의 간격)
align-content

flex-direction : row |row-reverse | column | column-reverse
/* column(세로정렬) 일 경우에는 justfy-conent, align-items로 바뀐다. */

flex-wrap : no-wrap | wrap | wrap-reverse
/* flex는 기본이 넘치지 않는다(자식의 width를 무시하면서 한 줄에 넣는다.) */
/* 자식요소가 넘치게 (여러줄) 하려면 wrap 속성을 주어야 한다.*/
...
```
[align-self 01.flex.html](../html/01.flex.html) => .wrapper4 참조
